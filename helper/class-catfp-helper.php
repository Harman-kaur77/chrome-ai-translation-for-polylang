...
if (! class_exists('CATFP_Helper')) {
	class CATFP_Helper
	{
		...
		{
			$response = wp_remote_get( esc_url_raw( CATFP_URL . 'includes/block-translation-rules/block-rules.json' ), array(
				'timeout' => 15,
			) );
			...
			if ( is_wp_error( $response ) || 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
				...
				$local_path = CATFP_DIR_PATH . 'includes/block-translation-rules/block-rules.json';
			...
			$custom_block_translation = get_option('catfp_custom_block_translation', false);
