...
const catfpUpdateWidgetContent = (translations) => {

    translations.forEach(translation => {
        // Find the model by ID using the catfpFindModelById function
        const model = catfpFindModelById(elementor.elements.models, translation.ID);
        if (model) {
...
const catfpUpdateMetaFields = (metaFields, service) => {
    const AllowedMetaFields = select('block-catfp/translate').getAllowedMetaFields();
        ...
        Object.keys(metaFields).forEach(key => {
            ...
            if (Object.keys(AllowedMetaFields).includes(key)) {
                const translatedMetaFields = select('block-catfp/translate').getTranslatedString('metaFields', metaFields[key][0], key, service);
...
const catfpUpdateTitle = (title, service) => {
    if(title && '' !== title){
        const translatedTitle = select('block-catfp/translate').getTranslatedString('title', title, null, service);
...
const catfpFindModelById = (elements, id) => {
    for (const model of elements) {
        if (model.get('id') === id) {
            return model;
        }
        const nestedElements = model.get('elements').models;
        const foundModel = catfpFindModelById(nestedElements, id);
        if (foundModel) {
            return foundModel;
        }
    }
...
const updateElementorPage = ({ postContent, modalClose, service }) => {
        ...
        if (typeof settings === 'object' && settings !== null) {
            ...
            Object.keys(settings).forEach(key => {
                ...
                if (subStringsToCheck(key) &&
                    ...
                    const uniqueKey = ids.join('_catfp_') + '_catfp_settings_catfp_' + key;

                    const translatedData = select('block-catfp/translate').getTranslatedString('content', settings[key], uniqueKey, service);
                        ...
                        if (typeof item === 'object' && item !== null) {
                            ...
                            Object.keys(item).forEach(repeaterKey => {
                                ...
                                if (subStringsToCheck(repeaterKey) &&
                                    ...
                                    const uniqueKey = ids.join('_catfp_') + '_catfp_settings_catfp_' + key + '_catfp_' + index + '_catfp_' + repeaterKey;

                                    const translatedData = select('block-catfp/translate').getTranslatedString('content', item[repeaterKey], uniqueKey, service);
    ...
    // Update widget content with translations
    catfpUpdateWidgetContent(translations);
    
    // Update Meta Fields
    catfpUpdateMetaFields(postContent.metaFields, service);

    // Update Title
    catfpUpdateTitle(postContent.title, service);

    const replaceSourceString=()=>{
        const elementorData = catfp_global_object.elementorData;
        const translateStrings=wp.data.select('block-catfp/translate').getTranslationEntry();
        ...
        translateStrings.forEach(translation => {
            ...
            const keyArray = ids.split('_catfp_');
    ...
    fetch(catfp_global_object.ajax_url, {
        ...
        .then(data => {
            if (data.success) {
                const translateButton = document.querySelector('.catfp-translate-button[name="catfp_meta_box_translate"]');
                if(translateButton){
