...
const StoreTimeTaken = ({ prefix = false, start = false, end = false, translateStatus = false }) => {
    ...
    if (prefix) {
        ...
        if (start && end) {
            const oldTimeTaken = select('block-catfp/translate').getTranslationInfo().translateData[prefix]?.timeTaken || 0;
        ...
        dispatch('block-catfp/translate').translationInfo(data);
    }
}
