...
const SaveTranslation = ({type, key, translateContent, source, provider, AllowedMetaFields}) => {
    ...
    if (['title', 'excerpt'].includes(type)) {
        ...
        dispatch('block-catfp/translate')[action](translateContent, provider);
    } else if (['metaFields'].includes(type)) {

        if(Object.keys(AllowedMetaFields).includes(key)){
            dispatch('block-catfp/translate').metaFieldsSaveTranslate(key, translateContent, source, provider);
        }
    } else {
        dispatch('block-catfp/translate').contentSaveTranslate(key, translateContent, source, provider);
    }
}
