...
const updateTranslatedContent = ({provider, startTime, endTime}) => {
    const container = document.getElementById("catfp_strings_model");
    const stringContainer = container.querySelector('.catfp_string_container');
    ...
    translatedData.forEach((ele, index) => {
        ...
        const translationEntry = select('block-catfp/translate').getTranslationInfo().translateData[provider];
...
const onCompleteTranslation = ({container,provider, startTime, endTime, translateStatus, modalRenderId}) => {
    const conainer=document.querySelector(`#catfp-${provider}-strings-modal.modal-container[data-render-id="${modalRenderId}"]`);
    ...
    container.querySelector(".catfp_translate_progress").style.display = "none";
    container.querySelector(".catfp_string_container").style.animation = "none";
...
const ModalStringScroll = (translateStatus,provider,modalRenderId) => {
    ...
    if (scrollHeight !== undefined && scrollHeight > 100) {
        document.querySelector(".catfp_translate_progress").style.display = "block";
