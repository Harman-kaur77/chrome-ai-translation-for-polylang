const FilterTargetContent = (props) => {
    ...
    const OpenSpanPlaceholder = '#catfp_open_translate_span#';
    const CloseSpanPlaceholder = '#catfp_close_translate_span#';
    const OpenTempTagPlaceholder = '#catfp_temp_tag_open#';
    const CloseTempTagPlaceholder = '#catfp_temp_tag_close#';
    const LessThanSymbol = '#catfp_less_then_symbol#';
    const GreaterThanSymbol = '#catfp_greater_then_symbol#';
    const entityOpenPlaceholder = '#catfp_entity_open_translate_span#';
    const entityClosePlaceholder = '#catfp_entity_close_translate_span#';
     ...
     * If the service is 'localAiTranslator', the content is filtered using filterSourceData function, otherwise, the content remains unchanged.
     */
    const content = ['localAiTranslator'].includes(props.service) ? filterSourceData(props.content) : props.content;
    ...
    return (
        <>
            {['localAiTranslator'].includes(props.service) ?
                content.map((data, index) => {
                    const notTranslate = notTranslatePattern.test(data);
                    if (notTranslate) {
                        return <span key={index} className="notranslate catfp-notraslate-tag" translate="no">{filterContent(data)}</span>;
                    } else {
                        return data;
                    }
                })
