...
const popStringModal = (props) => {
    ...
    const translateData = select('block-catfp/translate').getTranslationInfo().translateData[selectedService] || false;
    ...
    const translateStatusHandler = (status) => {
        ...
        const characterCount = select('block-catfp/translate').getTranslationInfo().translateData[service]?.targetCharacterCount || 0;
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            const stringModal = document.querySelector('.catfp_string_container');
            if (stringModal) {
    ...
    return (
        <> {popupVisibility &&
            <div id={`catfp-${props.service}-strings-modal`} className="modal-container" style={{ display: popupVisibility ? 'flex' : 'none' }} data-render-id={props.modalRender}>
