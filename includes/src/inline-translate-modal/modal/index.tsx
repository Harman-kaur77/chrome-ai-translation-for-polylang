...
const TranslatorModal: React.FC<TranslateModalProps> = ({value, onUpdate, pageLanguage, onModalClose, modalOpen}) => {
  ...
  return (isErrorModalOpen ? <ErrorModalBoxCompat message={langError} onClose={() => {setIsErrorModalOpen(false); setIsModalOpen(true)}} Title={__("Chrome built-in translator AI", 'chrome-ai-translation-for-polylang')}>
              ...
              {langError && langError !== "" && (
                <div className={styles.languageErrorButtonWrapper}>
                  <button className={styles.languageErrorButton} onClick={HandlerLanguageError}>
                    {__("Language Error Details", 'chrome-ai-translation-for-polylang')}
                  </button>
                </div>
              )}
...
window?.catfpInlineTranslation?.TranslatorModal=TranslatorModal;