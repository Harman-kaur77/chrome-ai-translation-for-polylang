...
const catfpElementorConfirmBox = {
    init: function() {
        this.pageTitleEvent=false;
        if(window.catfpElementorConfirmBoxData){
            this.createConfirmBox();
        }
    },

    createConfirmBox: function() {
        const sourceLangName=window.catfpElementorConfirmBoxData.sourceLangName;
        const targetLangName=window.catfpElementorConfirmBoxData.targetLangName;
        const confirmBox = jQuery(`<div class="catfp-elementor-translate-confirm-box modal-container" style="display:flex">
            <div class="modal-content">
            <p>
                ${sprintf(
                __("The original page in %s was built with Elementor. Its content has already been copied into this new %s version. You can now translate it with Elementor to keep the same design, or edit it with the Gutenberg editor.", "chrome-ai-translation-for-polylang"),
                sourceLangName,
                targetLangName
                )}
            </p>
            <div>
                ...
                ${__("Translate with Elementor", "chrome-ai-translation-for-polylang")}
                </button>
                <button data-value="no">
                ${__("Edit with Gutenberg", "chrome-ai-translation-for-polylang")}
                </button>
            </div>
            </div>
        </div>
    ...
    closeConfirmBox: function() {
        this.setPageTitle();
        const confirmBox = jQuery('.catfp-elementor-translate-confirm-box.modal-container');
        confirmBox.remove();
    },
    ...
    confirmTranslation: function(e) {
        this.setPageTitle();
        e.preventDefault();
        const postId=window.catfpElementorConfirmBoxData.postId;
        const targetLangSlug=window.catfpElementorConfirmBoxData.targetLangSlug;

        if(postId && targetLangSlug) {
            let oldData=localStorage.getItem('catfpElementorConfirmBox');
            ...
            localStorage.setItem('catfpElementorConfirmBox', JSON.stringify(data));
    ...
    setPageTitle: function() {
        ...
        if(window.catfpElementorConfirmBoxData.editorType !== 'classic') {
            return;
        }
